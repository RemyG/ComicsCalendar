<?php 

define('BASE_URL', 'http://comics.local/'); // Base URL including trailing slash (e.g. http://localhost/)

define('DEFAULT_CONTROLLER', 'issues'); // Default controller to load
define('ERROR_CONTROLLER', 'error'); // Controller used for errors (e.g. 404, 500 etc)

define('DEFAULT_AUTHOR', 'Rémy Gardette'); // Default author name, displayed as meta author
define('DEFAULT_TITLE', 'ComicsCalendar'); // Default page title, displayed in head > title
define('DEFAULT_DESCRIPTION', 'Default page description'); // Default page description, displayed as meta description

?>